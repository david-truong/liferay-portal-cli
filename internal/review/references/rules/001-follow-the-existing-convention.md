...
A violation is a change that breaks from the convention its surrounding code already follows, or that introduces a second, conflicting pattern into a file that had one. Code that follows its file's established convention is not a violation, even when that convention differs from another rule here. Treat a convention as established only when at least two existing occurrences in the same file agree, or — for a codebase-wide identifier — when `git grep` returns more occurrences of the existing form than of the proposed variant. Below that threshold, the file has no established pattern and the default rules here apply.
