...
A violation is an `Iterator` obtained only to drive a `while (iterator.hasNext())` loop, where an enhanced `for` loop over the collection works. It is not a violation when the loop body calls `iterator.remove()`, `iterator.set(...)`, or `iterator.add(...)`, or otherwise needs the iterator handle — an enhanced `for` loop cannot express those.
