...
A violation is a hyphenated prefix or compound modifier in prose, such as `long-running`, `multi-method`, `self-harm`, or `At-Risk`. It does not apply to code identifiers, command line flags, numeric ranges, URLs and file paths, ticket or version tokens (`LPD-12345`, `release-2024.q1`), or a hyphen that is part of an official proper noun or product name, where the hyphen is part of the token.
