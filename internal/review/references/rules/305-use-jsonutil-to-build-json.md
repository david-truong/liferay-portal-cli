...
A violation is JSON assembled by injecting `JSONFactory` or calling `JSONFactoryUtil`, or by creating and populating `JSONObject` or `JSONArray` by hand, where the `JSONUtil` helpers apply. Not a violation: a `JSONObject` or `JSONArray` populated inside a loop, or whose `put` calls are individually guarded by `if`, where one fluent `JSONUtil` expression cannot reproduce it; flag only a fixed, unconditional set of `put` or array-element calls.
