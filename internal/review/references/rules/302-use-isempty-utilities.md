...
A violation is a hand written `x == null || x.isEmpty()`, `x == null || x.length == 0`, or the negated `x != null && !x.isEmpty()`, where the corresponding `isEmpty` utility applies. This rule covers only `Collection`, `Map`, and array operands. A `String` operand is out of scope. When the operand type is not visible in the diff, do not flag.
