...
- Name a method annotated `@Scheduled` with a `scheduled` prefix (`scheduledSyncJiraIssues`, not `syncJiraIssues`), so the timer trigger is visible at the call site and in logs without opening the class to find the annotation. [109]
...
- For one implementation in a family of sibling implementations of a shared interface (an `ObjectActionExecutor`, an `MVCActionCommand`), name its test after the implementation class, not the behavior it exercises — a behavior-based name does not say which implementation's interface method is under test. [606]
